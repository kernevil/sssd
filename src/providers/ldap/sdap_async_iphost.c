                 ...
                 time_t now)
{
    ...
    ret = sysdb_attrs_primary_name(sysdb, attrs,
                                   opts->iphost_map[SDAP_AT_IPHOST_NAME].name,
                                   &name);
    if (ret != EOK) {
        DEBUG(SSSDBG_MINOR_FAILURE,
    ...
    ret = sysdb_attrs_get_el(attrs,
                             opts->service_map[SDAP_AT_IPHOST_USN].sys_name,
                             &el);
                   ...
                   bool purge)
{
    ...
    if (id_ctx->srv_opts && id_ctx->srv_opts->max_iphost_value && !purge) {
        state->filter = talloc_asprintf(
                ...
                id_ctx->opts->service_map[SDAP_OC_IPHOST].name,
                id_ctx->opts->service_map[SDAP_AT_IPHOST_NAME].name,
                id_ctx->opts->service_map[SDAP_AT_IPHOST_NUMBER].name,
                id_ctx->opts->service_map[SDAP_AT_IPHOST_USN].name,
