...
struct sbus_method_in_uuss_out_qus_state {
    struct _sbus_sss_invoker_args_uuss in;
    struct _sbus_sss_invoker_args_qus *out;
};

static void sbus_method_in_uuss_out_qus_done(struct tevent_req *subreq);

static struct tevent_req *
sbus_method_in_uuss_out_qus_send
    (TALLOC_CTX *mem_ctx,
     struct sbus_connection *conn,
     sbus_invoker_keygen keygen,
     const char *bus,
     ...
     const char * arg3)
{
    struct sbus_method_in_uuss_out_qus_state *state;
    struct tevent_req *subreq;
    struct tevent_req *req;
    errno_t ret;

    req = tevent_req_create(mem_ctx, &state, struct sbus_method_in_uuss_out_qus_state);
    if (req == NULL) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to create tevent request!\n");
        return NULL;
    }
    ...
    subreq = sbus_call_method_send(state, conn, NULL, keygen,
                                   (sbus_invoker_writer_fn)_sbus_sss_invoker_write_uuss,
                                   bus, path, iface, method, &state->in);
    if (subreq == NULL) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to create subrequest!\n");
        ret = ENOMEM;
    ...
    tevent_req_set_callback(subreq, sbus_method_in_uuss_out_qus_done, req);
...
static void sbus_method_in_uuss_out_qus_done(struct tevent_req *subreq)
{
    ...
    req = tevent_req_callback_data(subreq, struct tevent_req);
    state = tevent_req_data(req, struct sbus_method_in_uuss_out_qus_state);
...
static errno_t
sbus_method_in_uuss_out_qus_recv
    (TALLOC_CTX *mem_ctx,
     struct tevent_req *req,
     uint16_t* _arg0,
     uint32_t* _arg1,
...
{
    return sbus_method_in_uuss_out_qus_send(mem_ctx, conn, _sbus_sss_key_uuss_0_1_2_3,
        busname, object_path, "sssd.dataprovider", "resolverHandler", arg_dp_flags, arg_entry_type, arg_name, arg_address);
}
...
sbus_call_dp_dp_resolverHandler_recv
    (TALLOC_CTX *mem_ctx,
     ...
     const char ** _error_message)
{
    return sbus_method_in_uuss_out_qus_recv(mem_ctx, req, _dp_error, _error, _error_message);
}
